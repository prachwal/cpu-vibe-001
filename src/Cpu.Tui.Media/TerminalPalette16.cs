...
public static class TerminalPalette16
{
    public static readonly (ConsoleColor Color, byte R, byte G, byte B)[] Entries =
    [
    ...
    public static ConsoleColor Nearest(byte r, byte g, byte b)
    {
        ...
        foreach ((ConsoleColor color, byte pr, byte pg, byte pb) in Entries)
        {
            int dr = r - pr;
            int dg = g - pg;
            int db = b - pb;
    ...
    public static (byte R, byte G, byte B) ToRgb(ConsoleColor c) => c switch
    {
        ConsoleColor.Black => (0, 0, 0),
        ConsoleColor.DarkBlue => (0, 0, 128),
        ConsoleColor.DarkGreen => (0, 128, 0),
